...
type Options struct {
	KapsoClient  *kapso.Client
	GatewayCheck func(url string) error // if nil, uses default HTTP health check
}
...
func Run(cfg *config.Config, w io.Writer, opts *Options) error {
	...
	results = append(results, checkGateway(cfg, opts.GatewayCheck))
...
func checkGateway(cfg *config.Config, checkFn func(string) error) Result {
	...
	if !strings.HasPrefix(gwURL, "http://") && !strings.HasPrefix(gwURL, "https://") {
		return Result{
			Name:   "Gateway connectivity",
			Status: "FAIL",
			Detail: fmt.Sprintf("invalid gateway URL: %s (expected http:// or https://)", gwURL),
		}
	}
	...
	if checkFn != nil {
		...
		if err != nil {
			return Result{
				Name:   "Gateway connectivity",
				Status: "FAIL",
				Detail: fmt.Sprintf("%s -- %s", gwURL, err.Error()),
			}
		}
	} else {
		err := defaultGatewayCheck(gwURL)
		if err != nil {
			msg := err.Error()
...
func defaultGatewayCheck(gwURL string) error {
	client := &http.Client{Timeout: 5 * time.Second}
	resp, err := client.Get(strings.TrimRight(gwURL, "/") + "/health")
	if err != nil {
		return err
	}
